...
public:
    ...
    Mivector operator-(const Mivector& otro) const {
        Mivector resultado;
        // copia de otro para poder anular elementos ya usados
        Mivector disponibles = otro;
        for (int i = 0; i < tamano; i++) {
            bool encontrado = false;
            for (int j = 0; j < disponibles.tamano && !encontrado; j++) {
                if (datos[i] == disponibles.datos[j]) {
                    resultado.push_back(datos[i]);
                    // mueve el ultimo al lugar j y reduce tamano
                    disponibles.datos[j] = disponibles.datos[disponibles.tamano - 1];
                    disponibles.tamano--;
                    encontrado = true;
                }
            }
